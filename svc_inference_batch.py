...
# How to use
# uv run svc_inference_batch.py --config configs/base.yaml --model vits_pretrain/sovits5.0.pth --wave test_waves/ --spk configs/singers/singer0047.npy
...
if __name__ == '__main__':
    ...
    for file in tqdm.tqdm(waves, desc="svc"):
        os.system(
            f"uv run svc_inference.py --config {args.config} --model {args.model} --wave {wave_path}/{file} --ppg {out_path}/{file}.ppg.npy --spk {args.spk} --shift {args.shift}")
